...
Promise.all([d3.csv('data/cleaned_data2.csv')
]).then(data=>{
  let airbnbData = data[0];
  airbnbData.forEach(d=> d.price= +d.price);

  let roomTypes = new Set(airbnbData.map(d => d.room_type));
  ...
  let simulation  = d3.forceSimulation(airbnbData)
      .force('x',d3.forceX(d=> scaleX(d.price_x)).strength(1.0))
  ...
  let simulationRadial = d3.forceSimulation(airbnbData)
      .force('r', d3.forceRadial(d=>{
        if (d.price_x < 100) return 300;
        else if (d.price_x < 200) return 250;
        else if (d.price_x < 400) return 200;
        else if (d.price_x < 600) return 150;
        else return 100;
        ...
        function tick(){
            if(radialForce){
                ...
                c.transition()
                 ...
                 d3.select('.annotaiton').style('visibility','visible');
            } else{
